...
class MaruChamp(RamenBot):
    ...
    def play_soccar(self):
        ...
        if self.state == "kickoff start":
            ...
            # todo: don't do this.
            # just chase forever:
            self.prev_state = self.state = "do activity"
            self.current_activity = self.chase
            return

            # Choose an action based on circumstances:
            nb = ball.next_bounce(self.game_time)
            if abs(ball.vel.z) < 30:
                self.do(activities.CutShot)
                self.state = "do activity"
            elif (
                ball.vel.length() > 0
        ...
        elif self.state == "do activity":
            # Just keep at it till it's done:
            if self.current_activity is None:
                self.state = "soccar"
    ...
    def on_first_tick(self):
        self.current_activity = None
        self.chase = activities.ChaseSmarter(self)
